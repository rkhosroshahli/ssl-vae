...
scaler.fit(np.reshape(x_train, (x_train.shape[0], x_train.shape[1] * x_train.shape[2])))
...
clf = train_mlp(x_initial, y_initial, scaler)
pred_y = predict_mlp(clf, x_train, scaler=scaler)
c0_score = f1_score(pred_y, y_train, average='macro')
all_train_f1_scores.append(c0_score)
pred_y = predict_mlp(clf, x_test, scaler=scaler)
...
vae_save_file = f"vae_mnist_epochs{epochs}_batchsize{batch_size}_weights"
...
for i in range(100):
    ...
    pred_y_pseudo_clf, survived_index = predict_threshold_mlp(clf, recon_data, threshold=0.99, scaler=scaler)
    if len(x_vae_labeled) == 0:
    ...
    clf = train_mlp(temp_x_clf_data, temp_y_clf_data, scaler=scaler)
    pred_y = predict_mlp(clf, x_train, scaler=scaler)
    ...
    # Component 2
    pred_y_pseudo_clf, survived_index = predict_threshold_mlp(clf, x_unlabeled[:], threshold=0.99, scaler=scaler)
    if len(survived_index) == 0:
        print("Nothing is survived... breaking loop")
        break
    ...
    print("Success ratio for each set of data", selftraining_success_f1_scores_per_iteration)
    print("Success ratio for all data", selftraining_success_f1_scores_all)
